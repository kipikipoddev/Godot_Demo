...
public class Build_Entity_Controller
{
    ...
    private IEntity_Model Build_Entity_Handler(Create_Entity_Request req)
    {
        ...
        var armor = req.Resource.Armor.Select(r => new Amount_Model(r.Amount, r.Type)).ToArray();
    ...
    private IAction_Model Action_Mapper(Action_Resource resource)
    {
        ...
        else if (resource is Stun_Resource stun)
            return new Stun_Action_Model(stun);
        else if (resource is Stasis_Resource stasis)
            return new Stasis_Action_Model(stasis);
        return null;
    }
}